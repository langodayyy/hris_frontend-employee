...
> = ({ data, infoColor = "var(--chart-6)", selectedMonth, selectedYear }) => {
  ...
  // Data dianggap kosong jika array 'data' kosong atau semua 'workHours' bernilai 0
  const hasDataForChart =
    data && data.length > 0 && data.some((item) => item.workHours > 0);
  // --- AKHIR LOGIKA PENGECEKAN DATA KOSONG ---
    ...
    <Card>
      <CardHeader className="flex items-center gap-2 space-y-0 sm:flex-row">
        <div className="grid w-full gap-3 text-center sm:text-left ">
          <div className="">
            ...
            <CardDescription className="">{periodeLabel}</CardDescription>
          </div>
      ...
      <CardContent className="px-2 sm:px-6">
        {hasDataForChart ? (
          ...
          // --- Pesan "Tidak Ada Data" saat data kosong ---
          <div className="flex items-center justify-center w-full h-[250px]">
            ...
            <p className="text-gray-500 italic text-center">
              Your work hours data is not available yet.
            </p>
          </div>
        )}
      </CardContent>
