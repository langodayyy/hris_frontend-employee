...
//     useEffect(() => {
//     if (employeeData) {
//         if (employeeData.contract_type) setContractType(employeeData.contract_type);
//     }
//     }, [employeeData]);
...
//     //         const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/employees/${employeeData?.employee_id}?_method=PATCH`, {
//     //                 method: "POST",
//     //                 headers: {
//     //                     "Authorization": `Bearer ${Cookies.get("token-employee")}`,
         ...
         <Card className="flex mr-[20px] gap-[15px] rounded-[15px] border border-black/15 bg-white shadow-[0px_2px_2px_0px_rgba(0,0,0,0.25)] overflow-hidden">
            ...
            <div className="flex mx-[20px] gap-[10px]">
                ...
                <div className="flex flex-col flex-1 gap-[8px]">
                    ...
                    <span className="text-gray-600 border border-neutral-300 rounded-md px-4 py-3 overflow-hidden">{employeeData?.account_number ?? "-"}</span> 
                </div>
            </div>
            <div className="flex mx-[20px] gap-[10px]">
                <div className="flex flex-col flex-1 gap-[8px]">
                    <Label>Salary</Label>
                    <span className="text-gray-600 border border-neutral-300 rounded-md px-4 py-3 overflow-hidden">IDR {employeeData?.salary ?? "-"}</span> 
                </div>
                <div className="flex flex-col flex-1 gap-[8px]">
                    <Label>Contract Type</Label>
                    <span className="text-gray-600 border border-neutral-300 rounded-md px-4 py-3 overflow-hidden">{employeeData?.contract_type ?? "-"}</span> 
                </div>
            </div>
            <div className="flex mx-[20px] gap-[10px]">
                <div className="flex flex-col flex-1 gap-[8px]">
                    <Label>Join Date</Label>
                    <span className="text-gray-600 border border-neutral-300 rounded-md px-4 py-3 overflow-hidden">{employeeData?.join_date ?? "-"}</span> 
                </div>
                <div className="flex flex-col flex-1 gap-[8px]">
                    <Label>Exit Date</Label>
                    <span className="text-gray-600 border border-neutral-300 rounded-md px-4 py-3 overflow-hidden">{employeeData?.exit_date ?? "-"}</span> 
                </div>
            </div>
        </Card>
